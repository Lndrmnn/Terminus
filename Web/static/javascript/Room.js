...
function Room(roomname, introtext, roompic){
	...
	this.intro_text = (typeof introtext === 'undefined') ? "Esta es una habitación simple / This is a simple room": introtext;
	this.cmd_text = {"pwd": "Estás en / You are in " + this.room_name + "."};
	//for event handling
...
Room.prototype.cd = function(args){
	if (args.length > 1){
		return "You can't move to multiple locations. / No puedes moverte a múltiples ubicaciones a la vez.";
	} else if (args.length == 0){
		enterRoom(Home);
		return "You have come Home! / ¡Has vuelto a Home!\n" + current_room.intro_text;
	} else if (args[0] === "..") {
		if (this.parents.length >= 1){
			...
			return "You have moved to " + current_room.toString() + ". / Te has movido a " + current_room.toString() + ".\n" + current_room.intro_text;
		} else {
			return "You are at the first room. / Ya estás en la primera habitación.";
		}
	} else if (args[0] === "~"){
		enterRoom(Home);
		return "You have come Home! / ¡Has vuelto a Home!\n" + current_room.intro_text;
		...
		return "There is no room called " + args[0] + ". / No existe ninguna habitación llamada " + args[0] + ".";
	}
};
...
Room.prototype.less = function(args){
	if (args.length < 1){
		return "Pick a different item to less. / Elige un objeto válido para usar con less (o cat).";
	} else {
		item = args[0];
		for (var i = 0; i < this.items.length; i++){
		...
		return "There is no " + args[0] + " here. / No hay ningún " + args[0] + " aquí.";
	}
};
...
Room.prototype.man = function(args){
	if (args.length < 1){
		return "Must ask the man about something to receive a response. / Debes consultar sobre un comando específico en man para recibir respuesta.";
	} else {
		if (args[0] in man_pages){
			return man_pages[args[0]];
		}
		return "There is no man page for that command / No hay página de manual man para ese comando";
	}
};

Room.prototype.help = function(args){
	return "Type 'man <COMMAND>' to ask the man for help / Escribe 'man <COMANDO>' para consultar el manual del hechizo";
};
...
Room.prototype.mv = function(args){
	if (args.length != 2){
		return "You need to move item A to place B. Use mv [itemA] [placeB] / Necesitas mover el objeto A al lugar B. Usa mv [objetoA] [lugarB].";
	} else {
		...
		if ((item_name_to_move >= 0) && (this.childrenStringArray().indexOf(args[1]) >= 0)){
			...
			return "Moved " + args[0] + " to " + args[1] + ". / Moviste " + args[0] + " a " + args[1] + ".";
		} else {
			return "Must be a valid item and location to move it. / Debe ser un objeto y una ubicación válidos para moverlo.";
		}
	}
};
...
Room.prototype.rm = function(args){
	if (args.length < 1){
		return "You must remove a particular item / Debes especificar un objeto para remover con rm";
	} else {
		stringtoreturn = "";
			...
			if (this.getItemFromName(args[i]) != -1){
				if (this.getItemFromName(args[i]).valid_cmds.indexOf("rm") > 0){
					...
					if ("rm" in removedItem.cmd_text){
						...
						stringtoreturn += "You just removed " + args[i] + " / Has eliminado " + args[i] + "\n";
					}
				} else {
					stringtoreturn += "That item cannot be removed / Ese objeto no se puede eliminar";
				}
			} else {
				return "That's not a valid object to remove. / Ese no es un objeto válido para eliminar con rm.";
			}
		}
...
Room.prototype.grep = function(args){
	if (this.commands.indexOf("grep") > 0){
		...
		if (this.getItemFromName(args[1]) != -1){
			...
			return "Not a valid item to search in. / No es un objeto válido donde buscar con grep.";
		}
	}
	return "You haven't learned this spell yet. / Aún no has aprendido este hechizo.";
};

Room.prototype.touch = function(args){
	if (args.length < 1){
		return "You must touch something in particular. / Debes especificar un objeto para crearlo con touch.";
	} else {
		var createdItemsString = "";
		for (var i = args.length - 1; i >= 0; i--) {
			if (args[i].length > 0){
				this.addItem(new Item(args[i], "This is a " + args[i] + " / Este es un " + args[i]));
		...
		if (createdItemsString === ""){
			return "You haven't touched anything. Check your syntax. / No has creado nada con touch. Revisa tu sintaxis.";
		}
		return "You have just created " + createdItemsString + " / Has creado " + createdItemsString;
	}
	return "You haven't learned this spell yet. / Aún no has aprendido este hechizo.";
};
...
Room.prototype.cp = function(args){
	if (args.length != 2){
		return "Incorrect syntax. Ask the OldMan for help. / Sintaxis incorrecta. Consulta al OldMan o usa man cp para ayuda.";
	} else {
		...
		if (item_to_copy != -1){
			...
			return "Just copied " + item_to_copy_name + " into " + new_item_name + ". / Copiaste " + item_to_copy_name + " en " + new_item_name + ".";
		}
		return "No item of that name to copy. / No existe ningún objeto con ese nombre para copiar.";
	}
	return "You haven't learned this spell yet. / Aún no has aprendido este hechizo.";
};
...
Room.prototype.tellme = function(args){
	if (args[0] === "combo"){
		return "The combination is 'terminus' (without the quotes). / La combinación es 'terminus' (sin las comillas).";
	}
	return "Incorrect syntax. Ask the OldMan for help. / Sintaxis incorrecta. Consulta al OldMan o usa man tellme para ayuda.";
};
...
Room.prototype.mkdir = function(args){
	if (this.commands.indexOf("mkdir") > 0){
		if (args.length === 1){
			...
			return "New room " + args[0] + " created / Nueva habitación " + args[0] + " creada";
		}
		return "Incorrect syntax. Ask the OldMan for help. / Sintaxis incorrecta. Consulta al OldMan o usa man mkdir para ayuda.";
	}
	return "You have not learned this spell yet / Aún no has aprendido este hechizo";
};
...
Room.prototype.sudo = function(args){
	if (this.commands.indexOf("sudo") > 0){
		if (args[0] === "less" && args[1] === "Certificate"){
			...
			return "Wrong syntax. Read the instructions again. / Sintaxis incorrecta. Lee las instrucciones nuevamente.";
		}
	} 
	return "You cannot cast this spell here. / No puedes lanzar este hechizo aquí.";
}
...
Room.prototype.IHTFP = function(args){
	...
	return "This is not a valid spell. / Este no es un hechizo válido.";
}
	...
	if (this.commands.indexOf("add") > 0){
		if (args[0] === "MagicLocker"){
			...
			if (typeof this.cmd_text["add"] === 'undefined'){
				return "You just added the " + args[0] + " locker / Acabas de añadir el casillero " + args[0];
			}
			return this.cmd_text["add"];
		}
		else {
			return "No valid locker of that name. / No existe ningún casillero válido con ese nombre.";
		}
	}
	return "This is not a valid spell. / Este no es un hechizo válido.";
}
